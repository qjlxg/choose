...
def usage():
    ...
    print('\t\tif not input, get top 20 funds from all funds in recommended_cn_funds directory')
...
def main(argv):
    ...
    fundlist_files = glob.glob('recommended_cn_funds/fundlist-*.txt')
    if (len(fundlist_files) > 0):
        ...
        print('没有在 recommended_cn_funds 文件夹中找到基金列表文件！')
        sys.exit(1)
